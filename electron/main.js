...
function waitForServer(port, timeoutMs = 90_000, signal) {
  return new Promise((resolve, reject) => {
    ...
    if (signal) signal.addEventListener('abort', () => {
      finish(signal.reason instanceof Error ? signal.reason : new Error(String(signal.reason)));
    }, { once: true });
    ...
    function attempt() {
      ...
      const req = http.get(`http://127.0.0.1:${port}/`, (res) => {
        ...
        if (res.statusCode && res.statusCode < 500) {
          ...
          if (Date.now() >= deadline) {
            finish(new Error(`Server on port ${port} kept returning ${res.statusCode} after ${timeoutMs / 1000}s.`));
          } else {
      ...
      req.on('error', () => {
        if (done) return;
        if (Date.now() >= deadline) {
          finish(new Error(`Server on port ${port} did not respond within ${timeoutMs / 1000}s.`));
        } else {
...
function findNodeForVersion(major) {
  ...
  function nodeMajor(bin) {
    try {
      const raw = execFileSync(bin, ['--version'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  ...
  if (fs.existsSync(nvmVersionsDir)) {
    ...
    try {
      for (const entry of fs.readdirSync(nvmVersionsDir)) {
        ...
        const patch = parseInt(entry.replace(/^v/, '').split('.')[2] || '0', 10);
        const bin = path.join(nvmVersionsDir, entry, 'bin', 'node');
        if (patch > bestPatch && fs.existsSync(bin) && nodeMajor(bin) === major) {
          bestPatch = patch;
          bestBin = bin;
        }
      }
    } catch (_) { /* readdir failed */ }
...
function startServer(port) {
  ...
  if (IS_DEV) {
    ...
    const nextModuleBase = path.join(WEB_DIR, 'node_modules', 'next', 'dist', 'bin');
...
function loadingHtml() {
  ...
  var INF = 'M100,50 C100,25 82,6 56,6 C30,6 4,24 4,50 C4,76 30,94 56,94 C82,94 100,75 100,50 C100,25 118,6 144,6 C170,6 196,24 196,50 C196,76 170,94 144,94 C118,94 100,75 100,50 Z';
...
'\n' +
'    <!-- ① skeleton track -->\n' +
'    <path class="inf-track" d="' + INF + '"/>\n' +
'\n' +
'    <!-- ② outer glow snake -->\n' +
'    <path class="inf-outer" id="lOuter" d="' + INF + '"/>\n' +
'\n' +
'    <!-- ③ mid glow snake -->\n' +
'    <path class="inf-mid"   id="lMid"   d="' + INF + '"/>\n' +
'\n' +
'    <!-- ④ main crisp snake -->\n' +
'    <path class="inf-main"  id="lMain"  d="' + INF + '"/>\n' +
'\n' +
'    <!-- ⑤ bright specular snake -->\n' +
'    <path class="inf-bright" id="lBright" d="' + INF + '"/>\n' +
'\n' +
