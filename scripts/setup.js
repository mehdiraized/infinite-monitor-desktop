...
// ── 0. Check pnpm ────────────────────────────────────────────────────────────
console.log('  [0/5] Checking for pnpm...');
try {
...
// ── 1. Init submodule ────────────────────────────────────────────────────────
console.log('\n  [1/5] Initializing web/ submodule...');
run('git submodule update --init --recursive');

// ── 2. Upstream remote ───────────────────────────────────────────────────────
console.log('\n  [2/5] Configuring upstream remote in web/...');
const remotes = capture('git remote', WEB_DIR).split('\n').map(s => s.trim());
...
console.log('\n  [3/5] Checking for legacy npm node_modules in web/...');
const webNm = path.join(WEB_DIR, 'node_modules');
const pnpmDir = path.join(webNm, '.pnpm');
if (fs.existsSync(webNm) && !fs.existsSync(pnpmDir)) {
  console.log('  Found npm-managed node_modules in web/. Removing before pnpm install...');
  fs.rmSync(webNm, { recursive: true, force: true });
  console.log('  ✓  Removed web/node_modules — pnpm will recreate it with symlinks.');
} else {
...
console.log('\n  [4/5] Installing workspace dependencies...');
